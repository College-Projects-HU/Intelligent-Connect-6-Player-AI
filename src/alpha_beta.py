...
def alpha_beta(game, depth, alpha, beta, maximizing_player):
    win = game.check_win(game.last_row, game.last_col)
    if win[0] and win[1] == connect6.AI_PLAYER:
        return math.inf
    elif win[0] and win[1] == connect6.PLAYER_1:
        return -math.inf
    elif depth == 0 or game.check_draw():
    ...
    if maximizing_player:
        ...
        for (x1, y1) in game.get_all_legal_moves():
            game.board[x1][y1] = connect6.AI_PLAYER  # SET FIRST STONE
            game.last_row = x1
            game.last_col = y1

            for (x2, y2) in game.get_all_legal_moves():
                # SET SECOND STONE
                game.board[x2][y2] = connect6.AI_PLAYER
                game.last_row = x2
                game.last_col = y2
                ...
                # Reset second stone
                game.board[x2][y2] = connect6.EMPTY
                ...
                if beta <= alpha:
                    ...
                    break
            # Reset first stone
            game.board[x1][y1] = connect6.EMPTY
...
def find_best_move(game):
    ...
    for (x1, y1) in game.get_all_legal_moves():
        ...
        for (x2, y2) in game.get_all_legal_moves():
            game.board[x2][y2] = connect6.AI_PLAYER # SET SECOND STONE
            game.last_row = x2
            game.last_col = y2
